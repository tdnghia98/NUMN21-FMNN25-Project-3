...
def DN_iteration(nx, ny, theta = 0.7, maxiter = 100, tol = 1e-10, pltot = True):
    ...
    for k in range(maxiter):
        ...
        if rank == 2:
            ...
            comm.send(converged, dest=1)
            comm.send(converged, dest=3)
            if converged:
                break
            else:
        ...
        if rank == 1 or rank == 3:
            converged = comm.recv(source=2)
            if converged:
                break

    if pltot:
        if rank == 1:
            comm.send(room1.get_solution(), dest = 2)
        if rank == 3:
            comm.send(room3.get_solution(), dest = 2)
        if rank == 2:      
            ## currently assuming
            if dx != dy:
            ...
            A[1:n+1, 1:n] = comm.recv(source=1)
            A[n+1:2*n, 1:-1] = np.flip(room2.get_solution(),1)
            A[2*n:-1, n+1:-1] = comm.recv(source=3)
